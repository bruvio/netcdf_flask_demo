...
@app.callback(
    Output("new_scatter_plot", "children"),
    [Input("scatter_plot", "n_clicks")],
    [State("new_scatter_plot", "children")],
)
def update_scatter(n_clicks, children):
    ...
    new_scatter_plot = html.Div(
        ...
        children=[
            dcc.Graph(id={"type": "Scatter_Plot", "index": n_clicks}),
            dcc.Dropdown(
                id={"type": "Selection_variable_X", "index": n_clicks},
                options=[{"label": i, "value": i} for i in DATA.columns],
                value=None,
            ),
            dcc.Dropdown(
                id={"type": "Selection_variable_Y", "index": n_clicks},
                options=[{"label": i, "value": i} for i in DATA.columns],
                value=None,
            ),
        ],
...
@app.callback(
    Output({"type": "Scatter_Plot", "index": MATCH}, "figure"),
    [
